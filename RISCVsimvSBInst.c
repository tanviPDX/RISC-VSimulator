...
#define B_OFFSET3_FIELD 0x00000080
...
InstFields_t* BTypeInst(uint32_t current_instruction){
    InstFields_t* BFields;
    int imm1, imm2, imm3, imm4;

    BFields = malloc(sizeof(InstFields_t));
    BFields->imm = 0x0;

    BFields->funct = (current_instruction & B_WIDTH_FIELD) >> B_WIDTH_SHAMT;
    ...
    printf("Immediate field: %x \nSource register 1 addr: %x \nSource register 2 addr: %x \nFunction: %x\n", BFields->imm, BFields->rs1, BFields->rs2, BFields->funct);
    return BFields;
}

/************* Extracting S-type Instruction fields *************/
InstFields_t* STypeInst(uint32_t current_instruction){
    InstFields_t* SFields;
    int imm1, imm2;
    char imm1s[20];
    char imm2s[20];

    SFields = malloc(sizeof(InstFields_t));
    SFields->imm = 0x0;

    SFields->funct = (current_instruction & S_WIDTH_FIELD) >> S_WIDTH_SHAMT;
    ...
    printf("Immediate field: %x \nBase register addr: %x \nSource register addr: %x \nFunction: %x\n", SFields->imm, SFields->rs1, SFields->rs2, SFields->funct);
    return SFields;
}
...
InstFields_t* Decode(uint32_t current_instruction){
    uint32_t Opcode;
    InstFields_t* SFields;
    InstFields_t* BFields;
    ...
    switch(Opcode){
        case 0x23:  printf("\n*****S-type Instruction*****\n");
                    SFields = STypeInst(current_instruction);
                    break;
        case 0x63:  printf("\n*****B-type Instruction*****\n");
                    BFields = BTypeInst(current_instruction);
                    break;
        default:    printf("\n*****Other Instruction*****\n");
    }
}
...
int main(int argc, char *argv[])
{
    ...
    while(flag){
        ...
        InstFields = Decode(current_instruction); //function call to decode instruction
        pc = pc + 4;
        if(i == 10) flag = 0; //condition to exit loop
        i++;
    }
